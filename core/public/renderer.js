...
async function loadTokenStats() {
    try {
        ...
        const remaining = stats?.remainingLabel || 'sisa n/a';
        document.getElementById('token-stats').textContent = `${label} used | ${remaining}`;
    } catch {
        document.getElementById('token-stats').textContent = '0 used | sisa n/a';
    }
}
...
async function handleFileSelect(e) {
    ...
    if (textFiles.length > 0) {
        try {
            ...
            const result = await window.electronAPI.stageFiles(stagedPayload);
            selectedFiles.push(...(result.files || []));
            (result.warnings || []).forEach(showWarning);
            renderFilePreview();
        } catch (error) {
            showWarning(`File upload failed: ${error.message}`);
        }
    }
...
async function sendMessage(overrideText = null) {
    ...
    if (selectedFiles.length > 0) {
        ...
        selectedFiles.forEach(f => {
            const tokenText = f.tokenEstimate ? `, ~${f.tokenEstimate} tokens` : '';
            const truncatedText = f.truncated ? ', truncated' : '';
            filesContext += `\n- ${f.name} (${Math.ceil((f.size || 0) / 1024)}KB${tokenText}${truncatedText})`;
        });
    }
    const displayText = text + filesContext;
    const attachments = selectedFiles.map(file => ({ id: file.id, name: file.name }));

    chatArea.innerHTML += `
        <div class="message user">
            <div class="avatar user-avatar">ME</div>
            <div class="bubble user-bubble">${escapeHtml(displayText)}</div>
        </div>
    `;
    scrollToBottom();
    ...
    try {
        ...
        if (response.status === 'disconnected') {
            showInfo(`⚠️ ${response.warning || 'Provider unreachable.'}`);
        }
