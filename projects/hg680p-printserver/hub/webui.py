...
# ==========================
# HTML TEMPLATE (Epson Utility Style)
# ==========================
HTML = """
<!DOCTYPE html>
<html>
...
<body>
  ...
  <script>
    ...
    function renderDashboard(data) {
      ...
      var totalJobs = o.tracked_jobs || o.total_jobs || 0;
      var totalPages = o.tracked_pages || o.total_pages || 0;
      var activePrinter = o.top_printer || o.most_active || '-';
      ...
      html += '<div class="stat-item"><div class="stat-value">' + safe(activePrinter.printer || activePrinter) + '</div><div class="stat-label">Most Active</div></div>';
    ...
    function renderPrinters(printers, summary) {
      ...
      printers.forEach(function(p) {
        ...
        if (summary && summary.printers) {
          var printerUsage = summary.printers.find(function(x) { return x.printer === p.name; });
          if (printerUsage) {
            status = 'OK';
            statusCls = 'ok';
          }
    ...
    function renderJobs(data) {
      var jobs = data.recent_jobs || data.jobs || [];
      if (!jobs || !jobs.length) {
    ...
    function renderActivity() {
      api('/logs').then(function(data) {
        ...
        logs.forEach(function(l) {
          html += '<div style="padding:8px 12px;border-bottom:1px solid var(--line);font-size:13px;">';
          html += '<span style="color:var(--text-muted);font-size:11px;">' + safe(l.time || l.timestamp) + '</span>';
          html += ' <strong>' + safe(l.action || l.message) + '</strong>';
          if (l.detail) html += ': ' + safe(l.detail);
          html += '</div>';
        });
...
@app.route("/printers")
def printers():
    return jsonify({"printers": get_cups_printers()})
...
def ipp_ports():
    targets = scan_ippusb_targets()
    return jsonify({"ok": True, "ports": [t["port"] for t in targets], "targets": targets})

@app.route("/logs")
def logs():
    return jsonify({"ok": True, "logs": read_json_file(LOG_FILE, [])})
...
def dashboard_summary():
    ...
    payload["overview"]["detected_printers"] = len(get_cups_printers())
...
def v7_nozzle():
    ...
    log_activity(action="NOZZLE", printer=printer, result="OK" if res.get("ok") else "FAIL", detail=res.get("error"))
    return jsonify({"printer": printer, "engine": engine, "target": target, **res})
...
def v7_clean():
    ...
    target = ctx["target"]
    res = run_clean(engine, target, power)
    log_activity(action="CLEAN", printer=printer, result="OK" if res.get("ok") else "FAIL", detail=res.get("error"))
    return jsonify({"printer": printer, "engine": engine, "power": power, "target": target, **res})
...
def v7_reset_waste():
    ...
    if not confirm:
        return jsonify({"ok": False, "error": "Confirmation required"})
    ctx = build_printer_context(printer, data.get("model"))
    if not ctx:
        return jsonify({"ok": False, "error": "Printer not found"})
    target = ctx["target"]
    res = reink_reset_waste(target)
    log_activity(action="RESET_WASTE", printer=printer, result="OK" if res.get("ok") else "FAIL")
    return jsonify({"printer": printer, **res})
...
def api_ink_status():
    ...
    return jsonify({"ok": True, "mode": "status-fallback", "ink": parsed.get("ink", {}), "waste": parsed.get("waste"), "raw": parsed.get("raw"), "target": target})
...
def build_printer_context(printer_name, model=None):
    ...
    for p in cups_printers:
        if p.get("name") == printer_name:
            ...
            if "usb" in uri.lower():
                if "ipp" in uri.lower():
                    target = {"type": "ipp-usb", "uri": uri}
                else:
                    target = {"type": "usb", "uri": uri}
            elif "ipp" in uri.lower():
                m = re.search(r"ipp://([^:]+):(\d+)", uri)
                if m:
                    target = {"type": "ipp", "host": m.group(1), "port": int(m.group(2)), "uri": uri}
            break
    if not target:
        return None
    return {"printer": printer_name, "model": model, "target": target}

def snmp_status(host):
    try:
        ...
        iterator = getCmd(
            ...
            ContextData(),
            ObjectType(ObjectIdentity("1.3.6.1.2.1.43.18.1.1.8"))
        )
        ...
        for var_bind in var_binds:
            ink_levels[str(var_bind[0])] = str(var_bind[1])
        return {"ok": True, "host": host, "ink": ink_levels}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
def get_cached_status(printer, model, target):
    try:
        if prefers_reinkpy_usb(target):
            usb_res = reink_status_usb(target)
            if usb_res.get("ok"):
                return usb_res
        ...
        pass
    return {"ok": False, "error": "Status unavailable", "target": target}

# Helper to run clean
def run_clean(engine, target, power=False):
    if engine == "auto":
        if prefers_reinkpy_usb(target):
            res = reink_clean_usb(power=power)
            if res.get("ok"):
                return res
        if target.get("host"):
            res = reink_clean(target, power=power)
            if res.get("ok"):
                return res
            res = escp2_clean(target["host"], power)
            if res.get("ok"):
                return res
    elif engine == "reinkpy":
        return reink_clean(target, power=power)
    elif engine == "escp2":
        return escp2_clean(target.get("host", "127.0.0.1"), power)
    return {"ok": False, "error": "No engine available"}

# Helper to run nozzle check
def run_nozzle(engine, target):
    if engine == "auto":
        if prefers_reinkpy_usb(target):
            res = reink_nozzle_usb()
            if res.get("ok"):
                return res
        if target.get("host"):
            ...
            if res.get("ok"):
                return res
            res = escp2_nozzle_check(target["host"])
            if res.get("ok"):
                return res
    elif engine == "reinkpy":
